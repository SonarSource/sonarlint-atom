...
const lsDownloadUrl = 'https://repox.sonarsource.com/sonarsource/org/sonarsource/sonarlint/core/sonarlint-language-server/3.2.0.1446/sonarlint-language-server-3.2.0.1446.jar';

const lsFilename = 'sonarlint-ls.jar';
const bytesToMegabytes = 1024 * 1024;

const sonarjs = { name: 'SonarJS', filename: 'sonarjs.jar', downloadUrl: 'https://sonarsource.bintray.com/Distribution/sonar-javascript-plugin/sonar-javascript-plugin-4.1.0.6085.jar'};
const sonarphp = { name: 'SonarPHP', filename: 'sonarphp.jar', downloadUrl: 'https://sonarsource.bintray.com/Distribution/sonar-php-plugin/sonar-php-plugin-2.12.1.3018.jar'};
const sonarpython = { name: 'SonarPython', filename: 'sonarpython.jar', downloadUrl: 'https://sonarsource.bintray.com/Distribution/sonar-python-plugin/sonar-python-plugin-1.9.0.2010.jar'};
const sonarts = { name: 'SonarTS', filename: 'sonarts.jar', downloadUrl: 'https://sonarsource.bintray.com/Distribution/sonar-typescript-plugin/sonar-typescript-plugin-1.6.0.2388.jar'};
...
class SonarLintLanguageClient extends AutoLanguageClient {
  ...
  async activate(state) {
    ...
    if (!(pns.includes("atom-ide-ui") || pns.includes("linter"))) {
      await require("atom-package-deps").install("sonarlint", true);
    }
  ...
  installPlugin(pluginHome, plugin) {
    ...
    .then(() => { return DownloadFile(plugin.downloadUrl, localFileName, (bytesDone, percent) => this.updateInstallStatus(`downloading ${plugin.name} (${percent}% done)`));})
  ...
  installServer(serverHome) {
    ...
    .then(() => { return DownloadFile(lsDownloadUrl, localFileName, (bytesDone, percent) => this.updateInstallStatus(`downloading SonarLint language server (${percent}% done)`));})
