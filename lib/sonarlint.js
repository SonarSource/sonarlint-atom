...
class SonarLintLanguageServer extends AutoLanguageClient {
  ...
  spawnServer (command, args) {
    ...
    process.stdout.on('data', (data) => {
      console.log(`SonarLint Language Server stdout: ${data}`);
    });

    process.stderr.on('data', (data) => {
      console.log(`SonarLint Language Server stderr: ${data}`);
    });

    process.on('close', (code) => {
      console.log(`SonarLint Language Server process exited with code ${code}`);
    });
